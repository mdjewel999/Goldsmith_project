...
const ToyBookingRow = ({ booking, handleDelete, handleBookingConfirm }) => {
    ...
    return (
        ...
        <th>
            ...
            <button onClick={() => handleDelete(_id)} className="btn btn-active">Delate</button>
        </th>
        <td>
        ...
        <td>{service}</td>
        <td>{email}</td>
        <td>{rating}  <input type="radio" name="rating-2" className="mask mask-star-2 bg-orange-400" /></td>
        <td>${price}</td>
        <th>
          {status === "confirm" ? (
